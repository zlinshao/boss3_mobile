<template>
  <div id="uploadContainer">
    <div :id="ID">
      <input class="file_input" type="file" multiple accept @change="onClick"/>
    </div>
    ...
    methods: {
      getTokenMessage() {
        this.$http.get(globalConfig.server_user + 'files').then((res) => {
          ...
          this.uploadWithSDK(res.data.data, this.putExtra, this.config, globalConfig.domain);
        });

      },
      onClick() {
        this.uploadWithSDK(this.token, this.putExtra, this.config, globalConfig.domain);
      },
      // 初始化七牛
      uploadWithSDK(token, putExtra, config, domain) {
        let that = this;
        $('#' + this.ID).unbind("change").bind("change", function (event) {
          ...
          if (file) {
            let key = file.name;
            let board = that.addUploadBoard(file, config, key, "");
            if (!board) {
              return;
            }
            putExtra.params["x:name"] = key.split(".")[0];
            board.start = true;
            let dom_total = $(board).find("#totalBar").children("#totalBarColor");
            ...
            let complete = function (res) {
              $(board).find("#totalBar").addClass("hide");
              $(board).find(".control-container").html(
              ...
              if (res.key && res.key.match(/\.(jpg|jpeg|png|gif)$/)) {
                that.imageDeal(board, res.key, domain);
              }
            };
      ...
      addUploadBoard(file, config, key, type) {
        ...
        count > 1 && type != "3" ? "" : $(board).find(".resume").addClass("hide");
        return board;
      }
    }
  }
