<template>
  <div id="cardDetail">
    ...
    <div class="toContract"  v-if="this.process.place.name==='published'&&showEContract&&this.process.content.cookie===undefined&&(this.process.processable_type==='bulletin_rent_basic'||this.process.processable_type==='bulletin_rent_continued')" @click="toContract()"><i class="iconfont icon-hetong"></i></div>
            ...
            if (this.rentReport.indexOf(main.processable_type) > -1) {
              this.$http.get(this.urls + 'workflow/process/get/' + val).then(item => {
                ...
                if (main.place.status === 'published' && this.personalId.status) {
                  let bulletinArr = ['bulletin_rent_basic', 'bulletin_rent_continued', 'bulletin_rent_trans', 'bulletin_change', 'bulletin_rent_RWC', 'bulletin_RWC_confirm', 'bulletin_retainage'];
      ...
      newly() {
        ...
        switch (this.process.processable_type) {
          ...
          case 'bulletin_change'://调房
            ...
            break;
          case 'bulletin_lose_rent'://炸单
          case 'bulletin_lose_collect'://炸单
