  ...
  <div>
    <div class="banner">
      <div v-if="albumData.length<1&&detailData.house_goods&&detailData.house_goods.photo">
        <van-swipe :autoplay="0" :show-indicators="false" @change="onChange">
          <van-swipe-item v-for="(image, index) in detailData.house_goods.photo" :key="index">
    ...
    <div class="mainContent">
      <div class="content_top">
        <div class="address">
          <h3 style="font-size: .38rem;font-weight: bold">{{detailData.community.name}} {{detailData.door_address}}</h3>
          <div style="font-size: .2rem;color: #777">{{detailData.community.detailed_address}}</div>
          <div>
        ...
        <div class="rentStatus">
          ...
          <div style="font-size: .2rem;color: #777">已空置 {{detailData.current_ready_days}}天</div>
        </div>
      </div>

      <div class="content_other">
        <div>{{detailData.area}}</div>
        <div>{{detailData.floor.this}}/{{detailData.floor.all}}层</div>
        <div>{{detailData.house_type[0]}}室{{detailData.house_type[1]}}厅{{detailData.house_type[2]}}卫</div>
        <div>{{detailData.direction.name}}</div>
      </div>
        ...
        <van-row class="row_item">
          ...
          <van-col span="14">
            <van-row>
              ...
              <van-col span="12" class="value">{{detailData.staffs.name}}</van-col>
            </van-row>
          </van-col>
        </van-row>
        <van-row class="row_item">
          ...
          <van-col span="14">
            <van-row>
              ...
              <van-col span="12" class="value">{{detailData.department.name}}</van-col>
            </van-row>
          </van-col>
        </van-row>
        <van-row class="row_item">
          <van-col span="10">
            <van-row>
              ...
              <van-col span="16" class="value">{{detailData.property_type.name}}</van-col>
            </van-row>
          </van-col>
          <van-col span="14">
            <van-row>
      ...
      <div class="house_img">
        ...
        <div class="imgContent">
         <div class="imgItem" v-for="(item,index) in albumData" v-if="index<4"
          ...
          <div class="imgItem" v-if="albumData.length<4"  @click="showLargePic(detailData.house_goods.photo,0)">
      ...
      <div class="house_contract">
        <div class="contract_title">
          ...
          <div class="more" @click="router('contract',detailData.id)" v-if="lords.length>1 || renters.length>1">查看更多></div>
        </div>
        <div class="contractInfo" v-if="lords.length>0">
          <van-row class="contractItem">
            <van-col class="itemWord" span="6">合同编号 : </van-col>
            <van-col class="itemWord" span="18">{{lords[0].contract_number}}</van-col>
          </van-row>
          <van-row class="contractItem">
        ...
        <div class="contractInfo" v-if="renters.length>0">
          <van-row class="contractItem">
            <van-col class="itemWord" span="6">合同编号 : </van-col>
            <van-col class="itemWord" span="18">{{renters[0].contract_number}}</van-col>
          </van-row>
          <van-row class="contractItem">
            <van-col class="itemWord" span="6">合同状态 : </van-col>
            <van-col class="itemWord" span="18">
